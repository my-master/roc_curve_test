...
def roc_curve(y_true, y_prob):
    ...
    threshs = y_prob_sorted
    thresh_max = np.max(y_prob) + 1
    fp = 0
    tp = 0
    for i in range(threshs.size):
        if threshs[i] != thresh_max:
            thresh_max = threshs[i]
            fpr = fp / all_neg
            tpr = tp / all_pos
...
def plot_roc_curve(y_true, y_probas):
    ...
    for clf_name, probas in y_probas.items():
        rates = [(fpr, tpr) for fpr, tpr, _ in roc_curve(y_true, probas)]
